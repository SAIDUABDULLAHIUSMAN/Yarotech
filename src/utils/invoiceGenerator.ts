...
export const generateInvoicePDF = (
  ...
  autoTable(doc, {
    ...
    body: items.map((it, i) => [
      ...
      it.product_name,
      it.quantity,
      `₦${it.unit_price.toLocaleString("en-NG")}`,
      `₦${it.subtotal.toLocaleString("en-NG")}`,
    ]),
    theme: "grid",
    headStyles: { fillColor: primary, textColor: [255, 255, 255] },
    alternateRowStyles: { fillColor: [245, 247, 250] },
    bodyStyles: { fontSize: paperSize === "A5" ? 8 : 9 },
  ...
  doc.text(`₦${sale.total.toLocaleString("en-NG")}`, pageWidth - 20, finalY + 10, { align: "right" });

  // Signature section
  const sigY = finalY + 35;
  doc.setFont("helvetica", "normal");
  doc.setTextColor(0);
  doc.text("Authorized Signature:", pageWidth - 80, sigY);
  doc.line(pageWidth - 80, sigY + 2, pageWidth - 20, sigY + 2);

  if (signatureImageUrl) {
    try {
      doc.addImage(signatureImageUrl, "PNG", pageWidth - 75, sigY - 10, 50, 15);
    } catch (e) {
  ...
  doc.text("Generated by YaroTech Sales Manager", pageWidth / 2, footerY + 4, { align: "center" });
