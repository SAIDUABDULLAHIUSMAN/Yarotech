...
export async function generateInvoicePDF(
  ...
  const tableData = items.map((item, index) => [
    ...
    `${currencySymbol}${Number(item.unit_price).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`,
    `${currencySymbol}${Number(item.total_price).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`,
  ]);
  ...
  doc.text(
    `${currencySymbol}${Number(sale.total_amount).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`,
    pageWidth - 20,
    finalY + 22,
    { align: "right" }
  );
